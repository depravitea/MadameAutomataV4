...
const cmd = 
{
  data: new SlashCommandBuilder()
    ...
    .addUserOption(o=>o.setName("sub").setDescription("Assignee (leave empty for role blast)"))
    .addStringOption(o=>o.setName("title").setDescription("Title").setRequired(true))
    .addStringOption(o=>o.setName("details").setDescription("Details / checklist"))
    .addStringOption(o=>o.setName("deadline").setDescription("Relative time like 12h or 2d")),
  async run(i) {
    ...
    // Single sub path
    if (sub) {
      const t = await prisma.task.create({ data: { id: cuid(), ownerId: i.user.id, assigneeId: sub.id, title, details, dueAt } });
      const accept = new ButtonBuilder().setCustomId(`accept:${t.id}`).setStyle(ButtonStyle.Success).setLabel("Accept Task");
      ...
      try {
        ...
        await i.reply({ embeds: [gothicEmbed("Task Assigned", `Sent to <@${sub.id}> via DM.`)], ephemeral: true });
      } catch {
        await i.reply({ embeds: [gothicEmbed("Task Assigned", `**<@${sub.id}>** ${body}`)], components: [row] });
      }
      return;
    }
    // Role blast
    const cfg = await getConfig();
    if (!cfg.taskSlutRoleId) return i.reply({ content: "No TaskSlut role set. Use /setup.", ephemeral: true });
    const role = i.guild.roles.cache.get(cfg.taskSlutRoleId);
    if (!role) return i.reply({ content: "Configured TaskSlut role not found on this server.", ephemeral: true });

    const members = role.members.filter(m=>!m.user.bot);
    let sent = 0;
    for (const m of members.values()) {
      const t = await prisma.task.create({ data: { id: cuid(), ownerId: i.user.id, assigneeId: m.id, title, details, dueAt } });
      const accept = new ButtonBuilder().setCustomId(`accept:${t.id}`).setStyle(ButtonStyle.Success).setLabel("Accept Task");
      const row = new ActionRowBuilder().addComponents(accept);
      const body = `${title}\n${details||""}${dueAt?`\nDue: <t:${Math.floor(dueAt.getTime()/1000)}:R>`:""}`;
      try {
    ...
    await i.reply({ embeds: [gothicEmbed("Task Blast", `Assigned **${members.size}** tasks to <@&${role.id}>. DMs sent to **${sent}**.`)] });
  }
}
