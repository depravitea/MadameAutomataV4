...
export const data=new SlashCommandBuilder().setName('punish-muff')
  .setDescription('Timeout a user for N minutes (with a written reason)')
...
export async function execute(i:any){
  ...
  await prisma.punishment.create({ data:{ guildId:i.guildId!, targetId:t.id, kind:'muff', reason, createdBy:i.user.id } });
