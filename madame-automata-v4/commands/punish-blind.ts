...
export const data=new SlashCommandBuilder().setName('punish-blind')
  .setDescription('Blindfold toggle via role (with a written reason)')
...
export async function execute(i:any){
  ...
  await prisma.punishment.create({ data:{ guildId:i.guildId!, targetId:t.id, kind:'blind', reason, createdBy:i.user.id } });
