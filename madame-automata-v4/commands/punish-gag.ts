...
export const data=new SlashCommandBuilder().setName('punish-gag')
  .setDescription('Gag a user (bot will ignore) (with a written reason)')
...
export async function execute(i:any){
  ...
  await prisma.punishment.create({ data:{ guildId:i.guildId!, targetId:t.id, kind:'gag', reason, createdBy:i.user.id } });
