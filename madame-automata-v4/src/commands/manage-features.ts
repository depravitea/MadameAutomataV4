...
export async function execute(i:any){ const key=i.options.getString('key',true); const enabled=i.options.getBoolean('enabled',true); await setFlag(i.guildId!,key,enabled); await i.reply({ content:`Feature **${key}** is **${enabled?'enabled':'disabled'}**.` }); }
