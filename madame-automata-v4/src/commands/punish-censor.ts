...
export const data = new SlashCommandBuilder()
  .setName('punish-censor')
  .setDescription('Censor a user (bot will not respond to them)')
...
export async function execute(i: any) {
  ...
  await prisma.punishment.create({ data: { guildId: i.guildId!, targetId: t.id, kind: 'censor', createdBy: i.user.id } });
  await i.reply({ content: `${t} is censored from bot interactions.` });
}
